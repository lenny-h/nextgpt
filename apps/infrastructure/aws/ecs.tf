...
resource "aws_cloudwatch_log_group" "document_processor" {
  name              = "/ecs/${var.project_name}/document-processor"
  retention_in_days = 7
...
resource "aws_cloudwatch_log_group" "pdf_exporter" {
  name              = "/ecs/${var.project_name}/pdf-exporter"
  retention_in_days = 7

  tags = {
    Name = "${var.project_name}-pdf-exporter-logs"
  }
}
...
resource "aws_ecs_task_definition" "document_processor" {
  family                   = "${var.project_name}-document-processor"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "512"
  memory                   = "1024"
  ...
  tags = {
    Name = "${var.project_name}-document-processor-task"
  }
}

# PDF Exporter Task Definition
resource "aws_ecs_task_definition" "pdf_exporter" {
  family                   = "${var.project_name}-pdf-exporter"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "512"
  memory                   = "1024"
  ...
  tags = {
    Name = "${var.project_name}-pdf-exporter-task"
  }
}
...
resource "aws_ecs_service" "document_processor" {
  ...
  tags = {
    Name = "${var.project_name}-document-processor-service"
  }
}
...
resource "aws_ecs_service" "pdf_exporter" {
  ...
  tags = {
    Name = "${var.project_name}-pdf-exporter-service"
  }
}
