...
resource "aws_ecs_task_definition" "document_processor" {
  ...
  container_definitions = jsonencode([{
    ...
    image = "${local.ecr_document_processor_url}:latest"
    portMappings = [{
      containerPort = 8080
      protocol      = "tcp"
    }]
...
resource "aws_ecs_task_definition" "pdf_exporter" {
  ...
  container_definitions = jsonencode([{
    name  = "pdf-exporter"
    image = "${local.ecr_pdf_exporter_url}:latest"
    portMappings = [{
      containerPort = 8080
      protocol      = "tcp"
    }]
...
resource "aws_ecs_task_definition" "firecrawl_api" {
  ...
  container_definitions = jsonencode([{
    name  = "firecrawl-api"
    image = "${local.ecr_firecrawl_api_url}:latest"
    portMappings = [{
      containerPort = 8080
      protocol      = "tcp"
    }]
    environment = [
      ...
      },
      {
        ...
        value = "postgresql://postgres@${local.postgres_endpoint}/postgres"
      }
    ]
    logConfiguration = {
      logDriver = "awslogs"
...
resource "aws_ecs_task_definition" "firecrawl_playwright" {
  ...
  container_definitions = jsonencode([{
    ...
    image = "${local.ecr_firecrawl_playwright_url}:latest"
    portMappings = [{
      containerPort = 8080
      protocol      = "tcp"
    }]
